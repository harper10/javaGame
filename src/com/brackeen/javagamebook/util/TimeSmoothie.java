...
/**
 * Smoothes out the jumps in time do to poor timer accuracy. This is a simple
 * algorithm that is slightly inaccurate (the smoothed time may be slightly
 * ahead of real time) but gives better-looking results.
 */
public class TimeSmoothie {
	...
	/**
	 * Adds the specified time sample and returns the average of all the
	 * recorded time samples.
	 */
	...
	/**
	 * Gets the frame rate (number of calls to getTime() or addSample() in real
	 * time). The frame rate is recalculated every 500ms.
	 */
